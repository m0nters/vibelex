...
import { useGeminiTranslation } from "./useGeminiTranslation";
...
describe("useGeminiTranslation", () => {
  ...
  describe("initial state", () => {
    it("initializes with empty text, no translation, not loading, no error", () => {
      const { result } = renderHook(() => useGeminiTranslation());
  ...
  describe("translateText — loading state", () => {
    it("sets loading: true and stores text when translation starts", async () => {
      // Make processTranslation never resolve (hang) so we can inspect mid-flight
      vi.mocked(processTranslation).mockReturnValue(new Promise(() => {}));
  ...
  describe("translateText — success", () => {
    it("sets translation and clears loading on sentence translation", async () => {
      vi.mocked(processTranslation).mockResolvedValue(SENTENCE_OBJ);
    ...
    it("sets translation and clears loading on dictionary entry", async () => {
      vi.mocked(processTranslation).mockResolvedValue(DICTIONARY_OBJ);
  ...
  describe("translateText — error handling", () => {
    it("sets error and clears loading when processTranslation throws an AppException", async () => {
      const appError = new AppException({
        code: "API_KEY_INVALID",
        data: { message: "Bad key" },
      });
      vi.mocked(processTranslation).mockRejectedValue(appError);
    ...
    it("wraps a generic Error as GENERAL_ERROR AppException", async () => {
      vi.mocked(processTranslation).mockRejectedValue(
